...
public class OpenNettyService : IOpenNettyService
{
    ...
    /// <inheritdoc/>
    public virtual IAsyncEnumerable<(OpenNettyAddress Address, ImmutableArray<string> Values)> EnumerateDimensionsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        async IAsyncEnumerable<(OpenNettyAddress Address, ImmutableArray<string> Values)> ExecuteAsync(
    ...
    /// <inheritdoc/>
    public virtual IAsyncEnumerable<(OpenNettyAddress Address, OpenNettyCommand Command)> EnumerateStatusesAsync(
        OpenNettyProtocol protocol,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        async IAsyncEnumerable<(OpenNettyAddress Address, OpenNettyCommand Command)> ExecuteAsync(
            [EnumeratorCancellation] CancellationToken cancellationToken)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // A single dimension can never be returned for a non-null address that doesn't point to a specific device.
        //
        // As such, an exception is eagerly thrown if the address is a Nitoo/Zigbee address that doesn't
        // point to a unique device or if it is an SCS light point area, general or group address.
        switch (address?.Type)
        {
            ...
            case OpenNettyAddressType.Zigbee
                when OpenNettyAddress.ToZigbeeAddress(address.Value) is not { Identifier: not 0 }:
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // A single status can never be returned for a non-null address that doesn't point to a specific device.
        //
        // As such, an exception is eagerly thrown if the address is a Nitoo/Zigbee address that doesn't
        // point to a unique device or if it is an SCS light point area, general or group address.
        switch (address?.Type)
